      ...
      <div className="mb-6 flex shrink-0 items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          <div
            ...
            style={{
              background: 'color-mix(in srgb, var(--premium-accent-strong) 20%, transparent)',
        ...
        {showClose ? (
          <button type="button" className="premium-icon-btn shrink-0" aria-label="Close menu" onClick={onClose}>
            <X size={18} />
          </button>
        ) : null}
      </div>
...
export function PremiumAppShell({ children, hideSidebar = false, mainClassName = '' }: PremiumAppShellProps) {
  ...
  return (
    <div className="premium-app fixed inset-0 overflow-hidden">
      ...
      <div className={`relative grid h-full grid-cols-1 ${hideSidebar ? '' : 'lg:grid-cols-[260px_1fr]'}`}>
        {!hideSidebar ? (
          <>
            <aside
              className="hidden h-full shrink-0 flex-col overflow-hidden border-r p-5 backdrop-blur-xl lg:flex"
              style={{
                borderColor: 'var(--premium-sidebar-border)',
            ...
            {mobileNavOpen ? (
              <div className="fixed inset-0 z-[120] lg:hidden" role="dialog" aria-modal="true" aria-label="Navigation">
                <button
                  type="button"
                  className="premium-mobile-nav-scrim absolute inset-0"
                  aria-label="Close menu"
                  onClick={() => setMobileNavOpen(false)}
                />
                <aside
                  className="premium-mobile-drawer absolute inset-y-0 left-0 flex w-[min(20rem,calc(100vw-2.5rem))] flex-col overflow-hidden border-r p-5 shadow-2xl backdrop-blur-xl"
                  style={{
                    borderColor: 'var(--premium-sidebar-border)',
                    background: 'var(--premium-sidebar-bg)',
                  }}
        ...
        <div className="flex h-full min-h-0 min-w-0 flex-col overflow-visible">
          {!hideSidebar ? (
            <PremiumTopNav onOpenMenu={() => setMobileNavOpen(true)} menuOpen={mobileNavOpen} />
          ) : null}
          <main
            className={`premium-main relative z-0 min-h-0 flex-1 overflow-y-auto overscroll-y-contain ${mainClassName} ${
              hideSidebar ? 'flex flex-col items-center justify-start p-4 sm:p-6 md:p-8' : 'p-4 sm:p-5 md:p-7'
            }`}
          >
            {children}
          </main>
